<?php
//封装好的WeChat SDK，直接调用就行
class sendMsg{
	//关注时间以及纯文本回复
	function autoMsg($postObj,$Content){
    ...
    //关注图文回复或多图文回复
    function autoPics($postObj, $Articles){
        $toUsername = $postObj->FromUserName;
        $FromUsername = $postObj->ToUserName;
        $time = time();
        $msgType = "news";
    ...
    //点歌功能回复(语音，文字均可)
    function autoMusic($postObj,$msg){
    ...
    //天气查询功能
    function getWeather($postObj,$Content){
    ...
    //手机号码归属地查询
    function getPhone($postObj,$Content){
